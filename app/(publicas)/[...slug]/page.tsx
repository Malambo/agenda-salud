...
export default async function CategoriaPage({params}: {params: Promise<{slug: string}>}) {
    ...
    return (
        ...
        <div className='flex flex-col gap-24 items-center'>
            ...
            <p className='w-2/3 text-pretty text-lg'>
                {descripcion}
            </p>
        </div>

        <div className='mt-44'>
            {slug === 'centro-salud' && <p className='font-bold text-orange-500'>{slug.toLocaleUpperCase()}</p>}
            {slug === 'especialidades' && <TarjeteroEspecialidades />}
            {slug === 'profesionales' && <p className='font-bold text-orange-500'>{slug.toLocaleUpperCase()}</p>}
        </div>
        </>
    )
}
